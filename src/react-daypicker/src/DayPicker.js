...
export default class DayPicker extends Component {
  ...
  renderDay = (day, index) => {
    ...
    return (
      <td className={[ "day", isActive ? "active" : null, !day ? "empty" : null, isToday ? "today" : null ] .filter(v => v) .join(" ")} key={`${year}.${month}.day.${index}`} >
        <div>
          <p>{day ? day.getDate() : "" }</p> 
          {day ? <div> <Popup /> </div> : <div> </div>}
        </div>
      </td>
    );
    }
